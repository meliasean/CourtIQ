#!/usr/bin/env python3
"""
Score Canada Masters 2026 SF - both matches.

PROVENANCE
----------
Winners DERIVED from the Final pairing (DraftKings, 2026-08-13): Shelton vs
Nakashima. Each finalist maps to exactly one SF, so the mapping is unambiguous
...
Nakashima won as the +187 underdog; the book had Jodar at -233.

Usage:  python score_canada2026_sf.py [reports_dir]   (default: reports)
"""
import sys, os
import pandas as pd
import unicodedata
...
RESULTS = {
    ("Learner Tien", "Ben Shelton"):            "Ben Shelton",          # 1
    ("Rafael Jodar", "Brandon Nakashima"):      "Brandon Nakashima",    # 2
}
...
print("Scoring Canada Masters 2026 SF (2 matches)...")
score_file(os.path.join(reports, "canada2026_SF_predictions.csv"),
           os.path.join(reports, "canada2026_SF_predictions_complete.csv"))
cck = score_file(os.path.join(reports, "canada2026_SF_predictions_cck.csv"),
                 os.path.join(reports, "canada2026_SF_predictions_cck_complete.csv"))
if cck is not None:
