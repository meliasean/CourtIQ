#!/usr/bin/env python3
"""
Score Cincinnati 2026 R16 - all 8 matches.

PROVENANCE
----------
Winners DERIVED from the QF pairings (DraftKings, 2026-08-20), not read from
...
Four of eight favourites lost: Paul over Zverev (-281), Cobolli over Jodar
(-324), Tirante over Mensik (-208), Tiafoe over Auger-Aliassime (-222).
Book 4/8 this round.

Usage:  python score_cincinnati2026_r16.py [reports_dir]   (default: reports)
"""
import sys, os
import pandas as pd
import unicodedata
...
RESULTS = {
    ("Alexander Zverev", "Tommy Paul"):                  "Tommy Paul",               # 1
    ("Rafael Jodar", "Flavio Cobolli"):                  "Flavio Cobolli",           # 2
    ("Thiago Agustin Tirante", "Jakub Mensik"):          "Thiago Agustin Tirante",   # 3
    ("Arthur Fils", "Alex De Minaur"):                   "Arthur Fils",              # 4
    ("Taylor Fritz", "Christopher O'Connell"):           "Taylor Fritz",             # 5
    ("Nuno Borges", "Brandon Nakashima"):                "Brandon Nakashima",        # 6
    ("Jaime Faria", "Lorenzo Musetti"):                  "Lorenzo Musetti",          # 7
    ("Frances Tiafoe", "Felix Auger Aliassime"):         "Frances Tiafoe",           # 8
}
...
print("Scoring Cincinnati 2026 R16 (8 matches)...")
score_file(os.path.join(reports, "cincinnati2026_R16_predictions.csv"),
           os.path.join(reports, "cincinnati2026_R16_predictions_complete.csv"))
cck = score_file(os.path.join(reports, "cincinnati2026_R16_predictions_cck.csv"),
                 os.path.join(reports, "cincinnati2026_R16_predictions_cck_complete.csv"))
if cck is not None:
