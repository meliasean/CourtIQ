#!/usr/bin/env python3
"""
Re-score RG 2026 Final after regenerating predictions.

Usage:  python score_rg_f.py [reports_dir]   (default: reports)
"""
import sys, os
import pandas as pd
import numpy as np
...
# ---- KNOWN RESULTS ----
RESULTS = {
    ("Flavio Cobolli", "Alexander Zverev"): "Alexander Zverev",  # Zverev wins RG 2026
}
# -----------------------
...
def score_file(path, write_complete_to):
    ...
    for i, r in df.iterrows():
        ...
        if pd.isna(r.get("pred_winner")):
            print(f"  WARN row {i}: result known but no prediction — skipped")
            continue
...
# Try common round-code filenames since 'F' vs 'Final' varies
for round_code in ["F", "Final"]:
    std_path = os.path.join(reports, f"rg2026_{round_code}_predictions.csv")
    if os.path.exists(std_path):
        print(f"Scoring RG 2026 Final (round code: {round_code})...")
        std = score_file(std_path, os.path.join(reports, f"rg2026_{round_code}_predictions_complete.csv"))
        cck = score_file(os.path.join(reports, f"rg2026_{round_code}_predictions_cck.csv"),
                         os.path.join(reports, f"rg2026_{round_code}_predictions_cck_complete.csv"))
        if cck is not None:
    ...
    print("Could not find F or Final prediction file in", reports)
