#!/usr/bin/env python3
"""
Re-score RG 2026 QF after regenerating predictions.

Usage:  python score_rg_qf.py [reports_dir]   (default: reports)
"""
import sys, os
import pandas as pd
import numpy as np
...
RESULTS = {
    # === QF — all 4 matches ===
    ("Matteo Berrettini", "Matteo Arnaldi"): "Matteo Arnaldi",            # UPSET (Berrettini -181)
    ("Felix Auger Aliassime", "Flavio Cobolli"): "Flavio Cobolli",        # near coin flip, Cobolli wins
    ("Jakub Mensik", "Joao Fonseca"): "Jakub Mensik",                     # UPSET (Fonseca -224)
    ("Rafael Jodar", "Alexander Zverev"): "Alexander Zverev",
}
# ---------------------------------------------------------------------------
...
print("Scoring RG 2026 QF...")
std = score_file(os.path.join(reports, "rg2026_QF_predictions.csv"),
                 os.path.join(reports, "rg2026_QF_predictions_complete.csv"))
cck = score_file(os.path.join(reports, "rg2026_QF_predictions_cck.csv"),
                 os.path.join(reports, "rg2026_QF_predictions_cck_complete.csv"))
