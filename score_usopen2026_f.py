#!/usr/bin/env python3
"""
Score US Open 2026 Final.

RESULT (reported directly by Sean, 2026-09-13):
    Alexander Zverev def. Ben Shelton  -  Zverev takes the title.

Zverev was the -152 favourite, so the book was right. The model had Shelton at
54.7%, i.e. on the wrong side - and note that this prediction ran with
delta_rank = 1994, a fabricated value from the MISSING_RANK sentinel because
Zverev was absent from atp_rankings_current.csv (a Washington-scoped 32-player
snapshot). rank_diff is the model's #2 feature by importance. The prediction is
scored as logged; nothing is re-run.

Usage:  python score_usopen2026_f.py [reports_dir]   (default: reports)
"""
import sys, os
import pandas as pd
import unicodedata
...
RESULTS = {
    ("Alexander Zverev", "Ben Shelton"): "Alexander Zverev",
}
...
print("Scoring US Open 2026 Final...")
score_file(os.path.join(reports, "usopen2026_F_predictions.csv"),
           os.path.join(reports, "usopen2026_F_predictions_complete.csv"))
cck = score_file(os.path.join(reports, "usopen2026_F_predictions_cck.csv"),
                 os.path.join(reports, "usopen2026_F_predictions_cck_complete.csv"))
if cck is not None:
