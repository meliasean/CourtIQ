#!/usr/bin/env python3
"""
Score US Open 2026 QF - all 4 matches.

PROVENANCE
----------
Winners DERIVED from the SF pairings (DraftKings, 2026-09-10), not read from
...
Shelton beat Alcaraz as a +317 underdog. Book 3/4.

Usage:  python score_usopen2026_qf.py [reports_dir]   (default: reports)
"""
import sys, os
import pandas as pd
import unicodedata
...
RESULTS = {
    ("Alexander Zverev", "Botic van de Zandschulp"):   "Alexander Zverev",   # 1
    ("Karen Khachanov", "Alexander Blockx"):           "Karen Khachanov",    # 2
    ("Frances Tiafoe", "Alex Michelsen"):              "Frances Tiafoe",     # 3
    ("Ben Shelton", "Carlos Alcaraz"):                 "Ben Shelton",        # 4
}
...
print("Scoring US Open 2026 QF (4 matches)...")
score_file(os.path.join(reports, "usopen2026_QF_predictions.csv"),
           os.path.join(reports, "usopen2026_QF_predictions_complete.csv"))
cck = score_file(os.path.join(reports, "usopen2026_QF_predictions_cck.csv"),
                 os.path.join(reports, "usopen2026_QF_predictions_cck_complete.csv"))
if cck is not None:
