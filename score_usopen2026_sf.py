#!/usr/bin/env python3
"""
Score US Open 2026 SF - both matches.

PROVENANCE
----------
Winners DERIVED from the Final pairing (DraftKings, 2026-09-13): Zverev vs
Shelton. Each finalist maps to exactly one SF, so the mapping is unambiguous -
...
Both favourites held. Book 2/2.

Usage:  python score_usopen2026_sf.py [reports_dir]   (default: reports)
"""
import sys, os
import pandas as pd
import unicodedata
...
RESULTS = {
    ("Alexander Zverev", "Karen Khachanov"):   "Alexander Zverev",   # 1
    ("Frances Tiafoe", "Ben Shelton"):         "Ben Shelton",        # 2
}
...
print("Scoring US Open 2026 SF (2 matches)...")
score_file(os.path.join(reports, "usopen2026_SF_predictions.csv"),
           os.path.join(reports, "usopen2026_SF_predictions_complete.csv"))
cck = score_file(os.path.join(reports, "usopen2026_SF_predictions_cck.csv"),
                 os.path.join(reports, "usopen2026_SF_predictions_cck_complete.csv"))
if cck is not None:
